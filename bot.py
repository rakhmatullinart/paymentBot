...
def client_panel(message):
    bot.send_message(message.chat.id,'Выберите город:', reply_markup=markups.show_types(message.chat.id))
...
def handle_add_kat(call):
    sent = bot.send_message(call.message.chat.id, "Введите название города", reply_markup=markups.return_to_menu())
...
def handle_delete_kat(call):
    bot.edit_message_text("Выберите город для удаления", call.message.chat.id,
